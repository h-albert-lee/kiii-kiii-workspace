...
3. **생성 모델.** 주 코퍼스 문서 작성은 **GPT-6 Astra (`gpt-6-astra`)**로 선택(2026-09-15 한울 결정; 기존 후보 gpt-5.6-sol / claude-sonnet-5에서 변경). 파일럿에서 한국어 금융 문체 품질과 슬롯 준수를 검증한다. 검증용 20% 슬라이스는 **로컬 Qwen3.6-35B-A3B**로 동일 프롬프트 생성. 두 슬라이스 간 리더보드 순위 상관을 보고해 생성 모델 편향을 정량화한다.
