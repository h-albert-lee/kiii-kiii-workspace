...
8. **표면형 변형 축 (DLP 축)**: 정형 숫자 식별자는 regex로 풀리므로, 문서 단위 변형 레벨 T0(canonical)~T4(encoded) 5단계와 26개 변형 연산(한글 숫자 받아쓰기, 분할 발화, 부분 마스킹, OCR 혼동, 앵커 누락, 인코딩 등)을 정의한다. 스팬마다 `applied_ops`를 기록하고, F1을 (tier×kind)×(T-level) 격자로 보고한다. hard negative 6종을 gold의 ~30% 비율로 삽입해 precision을 의미 있게 만든다.
...
5. 변형 레벨 기본 분포(T0 25/T1 30/T2 20/T3 20/T4 5%)와 hard negative 비율(30%)의 적정성 — 파일럿 후 조정.
6. T4(encoded)를 본 벤치마크에 넣을지 별도 adversarial 트랙으로 뺄지 — 4페이지 분량 고려.
