...
- **Span extraction** scores all 36 categories, reported as micro-F1 overall and stratified by tier and kind (L-id / L-attr / I), following REDACT's sensitivity-tier reporting. Category-level scoring; subtypes are recorded but not scored in v1.
...
If every 주민등록번호 appeared as `900101-1234567` and every account as `110-123-456789`, a regex with a checksum would score near 1.0 and the leaderboard would measure nothing. Real AI-DLP inputs — chat logs, STT transcripts of call-centre audio, OCR'd scans, pasted tables, and deliberate exfiltration — carry the same values in forms no pattern anticipates. So `taxonomy.yaml: variation` defines four **levels** and 25 **operations**, each with an `applies_to` selector (numeric_id / text_id / attribute / specific ids) and a `regex_catchable` flag:
...
| T2 | lexical | Korean-numeral dictation (STT), mixed/Hanja digits, fullwidth/circled Unicode, OCR confusables, **STT digit errors and leftover spoken separators (에/다시)**, typos, name obfuscation (김O지, KMJ), romanisation, verbal amounts | no | hangul_digits, stt_digit_error, separator_artifact, unicode_variant, ocr_confusable, name_obfuscation |
| T3 | structural | value split across turns/lines/cells, **agent read-back (customer dictates in Hangul, agent repeats in digits)**, missing or wrong keyword anchor, coreference-only mentions, multi-subject interleaving, table layouts, negation/hypothetical | no | chunk_split, agent_readback, anchor_missing, anchor_wrong, coref_reference |

Each document is generated at one level (default mix T0 25 / T1 30 / T2 25 / T3 20 %); every gold span records its `applied_ops`, so recall can be analysed per operation. **Hard negatives** (six types: look-alike numbers, checksum-invalid values, public entities, dates shaped like RRN prefixes, placeholders, unattributed amounts) are injected at ~30 % of gold span count to make precision meaningful — a system that flags every 13-digit string should pay for it.

**Encoded evasion (base64, reversal, arithmetic hints, homoglyphs) is deliberately out of scope**: it is an attack distribution, not the input distribution of a DLP system, and it belongs to a separate adversarial track. Fullwidth/circled digits stay in T2 because IMEs and OCR produce them without adversarial intent.

The STT-facing operations are grounded in how Korean speech engines actually behave (`literature/notes/stt-korean-numbers.md`): ReturnZero applies inverse text normalisation by default, CLOVA and Google emit Arabic digits with no switch, so ~70 % of synthetic transcripts render numbers as digits; KsponSpeech and the AI Hub customer-service corpora keep a phonetic Hangul layer (`(3학년)/(삼 학년)`, `directText`), so ~20 % render them as `공일공 일이삼사 오육칠팔`; Whisper fragments numbers into tokens, so ~10 % are inconsistent. Spoken separators are `에` (which ReturnZero's ITN maps to a hyphen) and `다시` (a Japanese-derived "dash", per 국어원); zero is `공` or `영`. Crucially, **no commercial engine masks Korean PII at transcription time** — AWS Transcribe's redaction excludes ko-KR, Azure's MaskedITN masks profanity only — so masking happens downstream with regexes, and Hangul-dictated or split numbers pass straight through. A 20 % "pre-masked" subset reproduces exactly that leakage.

This gives the paper its third controlled variable alongside subject count and context length, and its most practitioner-relevant figure: rule/regex baselines vs. LLMs vs. Korean local LLMs as a function of T-level. The expected shape — rules collapse at T2, general LLMs hold through T2 and degrade at T3 (chunk_split, agent_readback, coref) — is a hypothesis to test, not a result to assume. Two ops deserve a sentence each in the paper: `partial_mask` (DLP systems routinely pass "already masked" strings that still leak birth date and gender) and `hangul_digits` (call-centre STT output is the dominant unstructured PII source in Korean finance, and it is invisible to every digit regex).

Prior art for the axis: Mind the Gap (Zafar & Nowaczyk 2026) has 7 OOD shift categories for English PII; REDACT stratifies by "disclosure form"; AmBench isolates name ambiguity. None cover dictated Korean numerals, STT digit errors, cross-turn splitting, agent read-back, or partial-mask leakage, and none tie variation to a legal tier.
